...
public class UserAuthenticationProvider implements AuthenticationProvider {
    ...
    public Authentication authenticate(Authentication authentication) throws AuthenticationException {
        ...
        if (!passwordEncoder.matches(password, dbPassword)) {
            log.info("[유저 페이지 - 로그인] 비밀번호가 일치하지 않습니다.");
            throw new BadCredentialsException("아이디 또는 비밀번호가 일치하지 않습니다.");
        }

        if (!user.getAuthority().getAuthority().equals(Authority.USER.getAuthority())) {
            log.info("[유저 페이지 - 로그인] 권한이 없습니다. 아이디 : {}, 권한 : {}", user.getUserId(),
                    user.getAuthority().getAuthority());
            throw new DisabledException("권한이 없는 계정입니다.");
        }

        if (user.getHasPermission().equals("N")) {
            log.info("[유저 페이지 - 로그인] 허가되지 않은 사용자입니다. 아이디 : {}, 권한 : {}", user.getUserId(),
                    user.getAuthority().getAuthority());
